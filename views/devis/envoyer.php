...
try {
    ...
    $mail->SMTPAuth = true;
    $mail->Username = 'votre_email@gmail.com'; // Your email
    ...
    $mail->addAddress($client['email'], $client['nom']); // Assuming client has an email field
    ...
    $mail->Body = "Bonjour {$client['nom']},<br><br>Veuillez trouver ci-joint votre devis.<br><br>Cordialement,<br>Votre Entreprise."; 

    // Send the email
    $mail->send();

    // Success message
    $_SESSION['devis_envoyee'] = "Devis envoyé avec succès à " . htmlspecialchars($client['nom']); 
    header("Location: index.php?entity=devis&action=index"); // Redirect to invoices page
    exit();
